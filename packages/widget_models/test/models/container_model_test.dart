...
import 'package:widget_models/widget_models/widgets/container_model.dart';

void main() {
  test("center model with no value test", () {
    final model = ContainerModel(id: "4947U#");

    expect(ModelType.container, model.type);
    expect(Container, model.toWidget().runtimeType);
    expect("Container()", model.toCode());
  ...
  test("center model with value test", () {
    ...
    expect(ModelType.container, model.type);
    expect(Container, model.toWidget().runtimeType);
    expect("Container(key:UniqueKey(),child:Center(),)", model.toCode());

    expect(sampleModelData, model.toJson());
  });

  test("center model from json", () {
    final model = ContainerModel.fromJson(sampleModelData);
